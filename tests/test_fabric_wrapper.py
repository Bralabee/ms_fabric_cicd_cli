...
class TestFabricCLIWrapper:
    ...
    def setup_method(self):
        ...
        self.fabric = FabricCLIWrapper("fake-token", telemetry_client=telemetry, validate_version=False)
    ...
    def test_create_workspace_success(self, mock_run):
        ...
        mock_create = Mock(stdout='{"id": "workspace-123", "displayName": "test-workspace"}', stderr='', returncode=0)
        
        mock_run.side_effect = [mock_exists, mock_create]
        
        # Use a GUID capacity to trigger the API path which returns JSON
        result = self.fabric.create_workspace("test-workspace", "00000000-0000-0000-0000-000000000000", "Test description")
    ...
    def test_create_workspace_already_exists(self, mock_run):
        ...
        mock_get = Mock(stdout='{"id": "workspace-123", "displayName": "test-workspace"}', stderr='', returncode=0)
    ...
    def test_create_lakehouse_with_folder(self, mock_run):
        ...
        workspace_response = Mock(stdout='{"id": "workspace-123"}', stderr='', returncode=0)
        folders_response = Mock(stdout='{"value": [{"id": "folder-456", "displayName": "test-folder"}]}', stderr='', returncode=0)
        create_response = Mock(stdout='{"id": "lakehouse-123", "displayName": "test-lakehouse"}', stderr='', returncode=0)
