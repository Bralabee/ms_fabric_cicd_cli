...
class TokenManager:
    ...
    def __init__(
        ...
        logger.info("TokenManager initialized with %ds refresh buffer", refresh_buffer_seconds)
    ...
    def _acquire_token(self) -> TokenInfo:
        ...
        try:
            ...
            expires_on = datetime.fromtimestamp(access_token.expires_on, tz=timezone.utc)
    ...
    def ensure_fresh_auth(self, max_age_seconds: float = 300) -> bool:
        ...
        if self._last_cli_auth:
            cli_auth_age = (datetime.now(timezone.utc) - self._last_cli_auth).total_seconds()
        
        needs_cli_refresh = (
            token_changed or
            cli_auth_age is None or
            cli_auth_age > max_age_seconds
        )
