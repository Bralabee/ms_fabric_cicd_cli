...
class DeploymentState:
    ...
    def start_deployment(self, deployment_id: Optional[str] = None) -> None:
        ...
        """
        self._deployment_id = deployment_id or datetime.now(timezone.utc).strftime(
            "%Y%m%d_%H%M%S"
        )
    ...
    def record(
        ...
        **metadata: Any,
        ...
        logger.debug(
            "Recorded: %s '%s' in workspace '%s'", item_type.value, name, workspace_name
        )
    ...
    def save_checkpoint(self, path: Optional[Path] = None) -> Path:
        ...
        if not checkpoint_path:
            checkpoint_path = Path(
                f".deployment_state_{self._deployment_id or 'unknown'}.json"
            )
    ...
    def _delete_item(
        self, fabric_wrapper: "FabricCLIWrapper", item: CreatedItem
    ) -> bool:
        """
