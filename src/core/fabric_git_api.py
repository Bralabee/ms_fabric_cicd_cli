...
class FabricGitAPI:
        ...
        if provider_type == GitProviderType.GITHUB:
            ...
            if credential_type == "ServicePrincipal":
                if not all([tenant_id, client_id]):
                    raise ValueError("ServicePrincipal requires tenant_id and client_id")
                ...
                raise ValueError("Azure DevOps only supports ServicePrincipal authentication")
        else:
    ...
    def connect_workspace_to_git(
        ...
        if provider_type == GitProviderType.AZURE_DEVOPS:
            if not all([organization_name, project_name, repository_name]):
                raise ValueError("Azure DevOps requires organization_name, project_name, and repository_name")
    ...
    def initialize_git_connection(self, workspace_id: str) -> Dict[str, Any]:
        ...
        try:
            ...
            logger.info(f"Initialized Git connection for workspace {workspace_id}. Required action: {required_action}")
            return {
                "success": True,
    ...
    def poll_operation(
        ...
        for attempt in range(max_attempts):
            try:
                ...
                logger.debug(f"Operation {operation_id} status: {status} (attempt {attempt + 1}/{max_attempts})")
        ...
        # Timeout
        logger.error(f"Operation {operation_id} timed out after {max_attempts * retry_after} seconds")
        return {
            "success": False,
