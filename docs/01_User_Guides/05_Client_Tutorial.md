...
**Example:**
To create a "Sales" project for client "Contoso" using the `medallion` template and Capacity ID `F64-12345`:
...
```bash
make destroy config=config/projects/contoso/sales.yaml
```

*Note: This operation permanently deletes the workspace and all associated data. Use `--force` (in the CLI) to skip the confirmation prompt.*

### E. Environment Promotion

Promote content through Fabric Deployment Pipeline stages:

```bash
# Promote from Development to Test
make promote pipeline="Contoso-Sales Pipeline" source=Development target=Test

# Promote from Test to Production
make promote pipeline="Contoso-Sales Pipeline" source=Test target=Production
```

*This uses the Deployment Pipeline created during onboarding. See the CLI Walkthrough for full details.*

### F. Full Onboarding (Single Command)

Bootstrap a complete environment (Dev + Test + Prod workspaces + Deployment Pipeline) with one command:

```bash
make onboard org="Contoso" project="Sales" template=medallion
```

*This automates Steps 1-3 above, plus creates Test/Prod workspaces and a Deployment Pipeline.*
