...
class CustomSolutionAnalyzer:
    ...
    def _analyze_function(self, node: ast.FunctionDef, file_path: Path):
        ...
        fabric_patterns = [
            "workspace", "lakehouse", "warehouse", "notebook", "pipeline",
            "folder", "item", "git", "capacity", "principal"
        ]
    ...
    def _determine_migration_complexity(self):
        ...
        cli_replaceable_count = len([c for c in self.analysis["components_found"] if c["cli_replaceable"]])
    ...
    def _generate_recommendations(self):
        ...
        total_loc = self.analysis["total_loc"]
        cli_replaceable = [c for c in self.analysis["components_found"] if c["cli_replaceable"]]
    ...
    def generate_report(self, output_file: str = None):
        ...
        report = {
            "analysis_summary": {
                ...
                "cli_replaceable": len([c for c in self.analysis["components_found"] if c["cli_replaceable"]]),
...
def main():
    parser = argparse.ArgumentParser(description="Analyze custom Fabric solution for migration")
    ...
    try:
        ...
        print(f"   CLI replaceable: {len([c for c in analysis['components_found'] if c['cli_replaceable']])}")
