...
Usage:
    PYTHONPATH=src python scripts/dev/test_promote_e2e.py
"""
import os
import sys
...
def main():
    ...
    print(f"  DeploymentStage.ORDER: {DeploymentStage.ORDER}")
    print(f"  FabricDeploymentPipelineAPI: OK")
    print(f"  get_environment_variables: OK")
    ...
    try:
        ...
        print(f"  FABRIC_TOKEN: {'ACQUIRED' if token else 'MISSING'} ({len(token)} chars)")
    ...
    if pipelines:
        ...
        if stages_result["success"]:
            ...
            for s in stages:
                ...
                print(f"    - {s.get('displayName', '?')} (id: {s.get('id', '?')[:8]}..., workspace: {ws or 'unassigned'})")
