...
app = typer.Typer(
    help="Create & initialize a GitHub repository for Fabric integration"
)
...
def create_repo(
    ...
    *,
    ...
    user_resp = requests.get(
        f"{GITHUB_API}/user", headers=_headers(token), timeout=30,
    )
    ...
    body: Dict[str, Any] = {
        ...
        "description": description or (
            "Fabric data-product repo – "
            "provisioned by usf-fabric-cli"
        ),
        "private": private,
        "auto_init": auto_init,
    }
    ...
    console.print(
        f"[blue]Creating GitHub repo "
        f"'{owner}/{repo_name}' …[/blue]"
    )
    resp = requests.post(
    ...
    if resp.status_code == 201:
        data = resp.json()
        console.print(
            f"[green]✅ Created: {data['html_url']}[/green]"
        )
        return data
    ...
    if resp.status_code == 422:
        ...
        if any(e.get("message") == "name already exists on "
               "this account" for e in errors):
            console.print(
                f"[yellow]⚠️  Repo '{owner}/{repo_name}' "
...
def ensure_branch(
    ...
    resp = requests.get(
        url, headers=_headers(token), timeout=30,
    )
    if resp.status_code == 200:
        console.print(
            f"[green]✓ Branch '{branch}' already exists[/green]"
        )
        return True
    ...
    console.print(
        f"[blue]Creating branch '{branch}' …[/blue]"
    )
    ...
    ref_resp = requests.get(
        ref_url, headers=_headers(token), timeout=30,
    )
    ...
    if create_resp.status_code in (201, 200):
        console.print(
            f"[green]✅ Branch '{branch}' created[/green]"
        )
        return True
...
def main(
    ...
    branch: str = typer.Option(
        "main", help="Branch to ensure exists",
    ),
    private: bool = typer.Option(
        True, help="Create as private repository",
    ),
):
    """
    ...
    if not token:
        console.print(
            "[red]GITHUB_TOKEN environment variable "
            "is required[/red]"
        )
        raise typer.Exit(1)
